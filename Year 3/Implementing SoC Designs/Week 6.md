...
It can also reduce power supply noise. In a synchronous circuit, logic begins to switch just after the active clock edge, gradually diminishing over time before the next clock due to the varying length of critical paths across the circuit. This periodically varying current draw actually creates a regular AC signal in the power wiring; this both acts as a transmitting aerial and potentially affects other gates' switching. If the entire chip is synchronous then this problem is at it's worst, but if there are several clocks with different phases then the demand tends to even out and noise is reduced.
The disadvantage of this approach is that signals must be synchronised when they arrive at their destination, which inherently adds some latency. In addition, this is somewhat unreliable without a long wait, so you might need to purposefully add even more latency to get an acceptable level of reliability. Therefore, GALS will absolutely slow down communications in some way.